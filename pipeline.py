...
Steps:
  1. ingest    — pull historical data from Ergast + FastF1
  2. warehouse — load Parquet files into DuckDB
  3. features  — engineer model features
  4. train     — train all three models
  5. predict   — smoke-test predictions on latest season
...
def step_warehouse() -> None:
    logger.info("═══ STEP 2: Build DuckDB warehouse ═══")
...
def step_predict() -> None:
    logger.info("═══ STEP 5: Smoke-test prediction ═══")
    import pandas as pd
    ...
    if sample.empty:
        logger.warning(f"No data for {VALIDATION_SEASON} round 1 — skipping smoke test.")
        return
...
def step_race_week() -> None:
    logger.info("═══ RACE WEEK: OpenF1 live pull ═══")
